...
@implementation LMSPlayerManager
...
static LMSPlayerManager *playerManager = nil;
+(instancetype)playerManager
{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        playerManager = [[LMSPlayerManager alloc] init];
    });
...
-(void)getPlayListCompletionHandler:(void(^)())handler {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        ...
        for (NSDictionary *dict in tempArr) {
            //创建Model对象
            LMSMusicInfo *musicInfo = [LMSMusicInfo new];
            //对应的向Model中的属性赋值
            [musicInfo setValuesForKeysWithDictionary:dict];
            [self.playlist addObject:musicInfo];
//            NSLog(@"%@", musicInfo.picUrl);
        }
//         NSLog(@"%@", tempArr);
        NSLog(@"%@", [[self.playlist firstObject] picUrl]);
...
-(LMSMusicInfo *)getmusicInfoWithIndext:(NSUInteger)index {
...
- (void)prepareMusic:(NSUInteger)index {
    ...
    if (self.currentIndex != index) {
        ...
        // 获取当前音乐信息
        LMSMusicInfo *musicInfo = [self getmusicInfoWithIndext:index];
        
        AVPlayerItem *playerItem = nil;
        
        if (musicInfo.auditionList.count != 0) {
            LMSAuditionList *music = [[musicInfo auditionList] objectAtIndex:0];
            // 实例化一个PlayerItem作为Player的"CD"
           playerItem  = [AVPlayerItem playerItemWithURL:[NSURL URLWithString:music.url]];
        } else if (musicInfo.mp3Url.length != 0) {
              playerItem = [AVPlayerItem playerItemWithURL:[NSURL URLWithString:musicInfo.mp3Url]];
        }
...
- (void)upMusic {
    ...
    NSString *index = [NSString stringWithFormat:@"%ld", self.currentIndex == 0 ? self.playlist.count - 1: self.currentIndex];
//    NSUserDefaults *musicIndex = [NSUserDefaults standardUserDefaults];
    [[NSUserDefaults standardUserDefaults] setObject:index forKey:@"musicIndex"];
}

// 播放下一首歌曲
- (void)nextMusic {
    [self prepareMusic:self.currentIndex + 1 < self.playlist.count ? self.currentIndex + 1 : 0 ];
    NSString *index = [NSString stringWithFormat:@"%ld", self.currentIndex < self.playlist.count ? self.currentIndex : 0];
    NSUserDefaults *musicIndex = [NSUserDefaults standardUserDefaults];
    [musicIndex setObject:index forKey:@"musicIndex"];
}
...
- (void)timerAction {
    ...
    if ([self.delegate respondsToSelector:@selector(didPlayChangeStatus:)]) {
        ...
        [self.delegate didPlayChangeStatus:[LMSMusicTimeFormatter getStringFormatBySeconds:currentTime]];
//         NSLog(@"~~%f",currentTime);
    }
}
